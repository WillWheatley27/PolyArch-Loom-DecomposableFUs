// fu_min_max_gen.sv -- GENUINE decomposable integer min/max, ONE shared core with
// capability tiers derived by parameter (SSOT). Unlike the earlier three separate
// designs (fu_min_max.sv monolithic 64, fu_min_max_decomp2.sv 2x32-only, and
// fu_min_max_decomp.sv full 4x16), every tier here is the SAME 4-block segmented
// comparator with identical ports; EN32/EN16 only gate which lane-boundary lex
...
//
//   mode: 00 -> 1x64, 01 -> 2x32, 10 -> 4x16  (a mode whose tier is disabled -> 1x64)
//   is_signed : 1 = signed compare, 0 = unsigned (held config, all lanes).
//   op_sel[i] : op of the lane whose start is 16-bit block i (0 min, 1 max); blocks
...
// Signedness only reinterprets the most-significant block of each lane.
// Combinational, latency 0.
module fu_min_max_dec #(
  ...
  // Enabled decompositions (EN* prunes disabled tiers to plain 1x64).
  logic use_2x32, use_4x16;
  assign use_2x32 = EN32 && (mode == M_2X32);
  assign use_4x16 = EN16 && (mode == M_4X16);
  ...
  assign top0 = use_4x16;
  assign top1 = use_2x32 || use_4x16;
  assign top2 = use_4x16;
  ...
  assign brk1 = use_4x16;
  assign brk2 = use_2x32 || use_4x16;
  assign brk3 = use_4x16;
  ...
  always_comb begin : route
    agt0 = r3; agt1 = r3; agt2 = r3; agt3 = r3;            // 1x64: one lane = r3
...
endmodule : fu_min_max_dec

// ---- Capability wrappers: identical ports, differ only in enabled tiers ----

module fu_min_max_m1 (                      // 64 only
  input  logic clk, input logic rst_n, input logic [1:0] mode, input logic is_signed, input logic [3:0] op_sel,
  input  logic [63:0] in_data_0, input logic in_valid_0, output logic in_ready_0,
  input  logic [63:0] in_data_1, input logic in_valid_1, output logic in_ready_1,
  output logic [63:0] out_data, output logic out_valid, input logic out_ready
);
  fu_min_max_dec #(.EN32(0), .EN16(0)) core (.*);
endmodule

module fu_min_max_m2 (                      // 64 + 2x32
  input  logic clk, input logic rst_n, input logic [1:0] mode, input logic is_signed, input logic [3:0] op_sel,
  input  logic [63:0] in_data_0, input logic in_valid_0, output logic in_ready_0,
  input  logic [63:0] in_data_1, input logic in_valid_1, output logic in_ready_1,
  output logic [63:0] out_data, output logic out_valid, input logic out_ready
);
  fu_min_max_dec #(.EN32(1), .EN16(0)) core (.*);
endmodule

module fu_min_max_m4 (                      // 64 + 2x32 + 4x16
  input  logic clk, input logic rst_n, input logic [1:0] mode, input logic is_signed, input logic [3:0] op_sel,
  input  logic [63:0] in_data_0, input logic in_valid_0, output logic in_ready_0,
  input  logic [63:0] in_data_1, input logic in_valid_1, output logic in_ready_1,
  output logic [63:0] out_data, output logic out_valid, input logic out_ready
);
  fu_min_max_dec #(.EN32(1), .EN16(1)) core (.*);
endmodule
