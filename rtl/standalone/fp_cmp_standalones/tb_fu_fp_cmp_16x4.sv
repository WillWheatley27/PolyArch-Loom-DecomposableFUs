// Self-checking Verilator TB for standalone 4xFP16 compare.
module tb_fu_fp_cmp_16x4 #(parameter int unsigned NRAND = 20000);
  logic [3:0] pred; logic [63:0] a,b,y;
  logic in_valid_0,in_valid_1,in_ready_0,in_ready_1,out_valid,out_ready; integer errors;
  fu_fp_cmp_16x4 dut (.clk(1'b0),.rst_n(1'b1),.pred(pred),.in_data_0(a),.in_valid_0(in_valid_0),.in_ready_0(in_ready_0),.in_data_1(b),.in_valid_1(in_valid_1),.in_ready_1(in_ready_1),.out_data(y),.out_valid(out_valid),.out_ready(out_ready));
  ...
  function automatic logic gold_bit(input int EW,input int MW,input logic [3:0] p,input logic [63:0] x,input logic [63:0] z);
    ...
    begin
      mask=(64'd1<<(EW+MW))-1;xa=x&mask;za=z&mask;expa=(x>>MW)&((64'd1<<EW)-1);mana=x&((64'd1<<MW)-1);expz=(z>>MW)&((64'd1<<EW)-1);manz=z&((64'd1<<MW)-1);sx=x[EW+MW];sz=z[EW+MW];uno=((expa==((64'd1<<EW)-1))&&(mana!=0))||((expz==((64'd1<<EW)-1))&&(manz!=0));
  ...
  function automatic logic [63:0] golden(input logic [3:0] p,input logic [63:0] x,input logic [63:0] z);logic [63:0] r;begin r='0;for(int i=0;i<4;i++)r[i*16+:16]={16{gold_bit(5,10,p,x>>(i*16),z>>(i*16))}};golden=r;end endfunction
  ...
  initial begin
    ...
    for(int p=0;p<16;p++)begin check(p,64'h3c00_c000_7e00_0000,64'h4000_bc00_3c00_8000);check(p,64'h7c00_fc00_0000_8000,64'h7c00_fc00_8000_0000);check(p,64'h0000_8000_0000_8000,64'h0000_0000_0000_0000);check(p,64'h0001_8001_7bff_fbff,64'h8001_0001_fbff_7bff);end
    ...
    for(int i=0;i<NRAND;i++)check($urandom[3:0],$urandom,$urandom);if(errors==0)$display("PASS: fu_fp_cmp_16x4, %0d random vectors",NRAND);else begin $display("FAIL: fu_fp_cmp_16x4 %0d errors",errors);$fatal(1);end;$finish;
  end
endmodule : tb_fu_fp_cmp_16x4
