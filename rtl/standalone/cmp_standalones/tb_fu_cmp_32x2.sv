// Self-checking Verilator TB for standalone 2x32 integer compare.
module tb_fu_cmp_32x2 #(parameter int unsigned NRAND = 20000);
  logic [3:0] pred;
  logic [63:0] a, b, y;
  ...
  function automatic logic [63:0] golden(input logic [3:0] p,
    ...
    begin
      r = '0;
      for (int l = 0; l < 2; l++)
        r[l*32 +: 32] = {32{cmp_bit(32, p, x >> (l*32), z >> (l*32))}};
      golden = r;
    end
  endfunction
  ...
  task automatic check_vec(input logic [3:0] p, input logic [63:0] x,
    ...
    begin
      ...
      exp = golden(p, x, z);
      if (y !== exp) begin
        $display("FAIL data: pred=%0d a=%h b=%h got=%h exp=%h", p, x, z, y, exp);
        errors++;
      end
  ...
  initial begin
    ...
    for (int p=0; p<10; p++) begin
      check_vec(p[3:0], 64'h0000_0000_0000_0005, 64'h0000_0000_0000_0003);
      check_vec(p[3:0], 64'hFFFF_FFFF_8000_0000, 64'h0000_0000_7FFF_FFFF);
      check_vec(p[3:0], 64'h0000_0000_0123_4567, 64'h0000_0000_0123_4567);
    end
    check_vec(4'd15, 64'hFFFF_FFFF_FFFF_FFFF, 64'd0);
    check_vec(4'd8, 64'hFFFF_FFFF_0000_0001, 64'h0000_0001_FFFF_FFFE);
    ...
    if (errors==0) $display("PASS: fu_cmp_32x2, %0d random vectors", NRAND);
    else begin $display("FAIL: fu_cmp_32x2 %0d errors", errors); $fatal(1); end
    $finish;
  end
endmodule : tb_fu_cmp_32x2
