// fu_cmp_8x8.sv -- Standalone fixed-width 8x8 integer compare baseline.
//
// Eight independent 8-bit lanes are packed in a 64-bit word.  pred selects
...
module fu_cmp_8x8 (
  ...
  // verilator lint_off UNUSEDSIGNAL
  logic lane_lt [0:7], lane_eq [0:7], lane_gt [0:7];
  logic lane_le [0:7], lane_ge [0:7], lane_ne [0:7];
  // verilator lint_on UNUSEDSIGNAL
  logic lane_gt_s [0:7], lane_result [0:7];

  for (genvar i = 0; i < 8; i++) begin : lane
    DW01_cmp6 #(.width(8)) u_cmp (
      .A(in_data_0[i*8 +: 8]), .B(in_data_1[i*8 +: 8]), .TC(1'b0),
      .LT(lane_lt[i]), .GT(lane_gt[i]), .EQ(lane_eq[i]),
      .LE(lane_le[i]), .GE(lane_ge[i]), .NE(lane_ne[i])
    );
    assign lane_gt_s[i] = (in_data_0[i*8+7] ^ in_data_1[i*8+7])
                          ? ~in_data_0[i*8+7] : lane_gt[i];
    assign lane_result[i] = pred_eval(pred, lane_gt[i], lane_gt_s[i], lane_eq[i]);
    assign out_data[i*8 +: 8] = {8{lane_result[i]}};
  end
endmodule : fu_cmp_8x8
